...
#define UART_IRQ_NUM                IRQ_UART0_SOURCE
#elif (UART_PORT == UART_PORT1)
#define UART_BASE                   UART1_BASE_ADDR
#define UART_IRQ_NUM                IRQ_UART1_SOURCE
#endif
...
#if (UART_PORT == UART_PORT0)
	/* set gpio 21 = mode 0, gpio 22 = mode 0 */
...
#ifdef CONFIG_SYS_FPGA
	/* set gpio 6 = mode 4, gpio 7 = mode 4 */
	pmx &= ~0xFF000000;
	pmx |= 0x44000000;
#else
	/* set gpio 0 = mode 0, gpio 1 = mode 0 */
	pmx &= ~0x000000FF;
#endif
...
char uart_rx(void)
{
	/* should be called from ISR to get the received buffer byte */
	uint32_t c;
	c = readl(UART_BASE + OFT_ATCUART_RBR);
	return c;
}
