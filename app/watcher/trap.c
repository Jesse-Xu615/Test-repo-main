...
extern void wlan_dtim_int_handler(void);
extern void rtc_int_handler(void);
extern void uart_int_handler(void);
extern void systimer_int_handler(void);
...
void watcher_trap_entry(void) __attribute__ ((interrupt ("machine") , aligned(4)));
void watcher_trap_entry(void)
{
	long mcause = read_csr(NDS_MCAUSE);
	long mepc = read_csr(NDS_MEPC);
	long mstatus = read_csr(NDS_MSTATUS);
#if CONFIG_SUPPORT_PFT_ARCH
	...
	if ((mcause & MCAUSE_INT) && ((mcause & MCAUSE_CAUSE) == IRQ_M_EXT)) {
		...
		if (irq_source == IRQn_WLAN0) {
			wlan_dtim_int_handler();
		} else if (irq_source == IRQn_RTC_ALARM) {
			rtc_int_handler();
		} else if (irq_source == IRQn_UART2) {
			uart_int_handler();
		} else if (irq_source == IRQn_BLE_TIMER) {
			systimer_int_handler();
		}
